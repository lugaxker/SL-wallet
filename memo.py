...
# Action codes
SET_PROFILE_NAME = 0x6d01
POST = 0x6d02
REPLY = 0x6d03
LIKE_AND_TIP = 0x6d04
SET_PROFILE_TEXT = 0x6d05
FOLLOW_USER = 0x6d06
UNFOLLOW_USER = 0x6d07
SET_PROFILE_PICTURE = 0x6d0a
POST_IN_TOPIC = 0x6d0c
FOLLOW_TOPIC = 0x6d0d
UNFOLLOW_TOPIC = 0x6d0e
CREATE_POLL = 0x6d10
ADD_POLL_OPTION = 0x6d13
VOTE_IN_POLL = 0x6d14
...
def create_memo_script( prefix, content ):
    ...
    if prefix in (SET_PROFILE_NAME, POST, SET_PROFILE_TEXT, SET_PROFILE_PICTURE,
                  POST_IN_TOPIC, FOLLOW_TOPIC, UNFOLLOW_TOPIC):
        return ( push_data(prefix_bytes) +
                 bytes().join( push_data( d.encode('utf-8') ) for d in content ) )
    elif prefix in (REPLY, LIKE_AND_TIP, ADD_POLL_OPTION, VOTE_IN_POLL):
        return ( push_data( prefix_bytes ) + push_data( bytes.fromhex( content[0] )[::-1] ) +
                 bytes().join( push_data( d.encode('utf-8') ) for d in content[1:] ) )
    elif prefix in (FOLLOW_USER, UNFOLLOW_USER):
        return ( push_data(prefix_bytes) +
                 push_data( bytes.fromhex( content[0] ) ) +
                 bytes().join( push_data( d.encode('utf-8') ) for d in content[1:] ) )
    elif prefix == CREATE_POLL:
        ...
        raise Exception("cannot serialize memo script")
...
def parse_memo_script( script ):
    ...
    if prefix in (SET_PROFILE_NAME, POST, SET_PROFILE_TEXT, SET_PROFILE_PICTURE,
        ...
        raise Exception("cannot deserialize memo script")
